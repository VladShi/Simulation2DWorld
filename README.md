<h1 style="text-align: center;">Проект “Симуляция”</h1>

#### (Делалось с целью самообучения java)
...
### Реализация
### `Общие комментарии`
<img src="./readme_assets/2024-11-03-simulation2dworld-ezgif.gif" width="350" height="324" alt="Пример программы"></img>
<br>Программа не имеет графического интерфейса, рендерится в консоль. <br>
Структура программы соответствует той что указана в [техническом задании](#техническое-задание). <br>
...
начале каждого хода проверяется есть ли ещё цель по координатам конечной точки пути из "path". Если 
...
Получена практика в написании алгоритма поиска в ширину. Использование дженериков. Работа с коллекциями - очередь,
двунаправленная очередь, хэш таблицы. ООП.

### `Запуск симуляции`
Для запуска необходима java версии 17+. \
...
Или собрать в единый jar файл, и запустить его. Что бы собрать jar файл находясь в папке **/bin** выполнить:

```
jar cfe Simulation2D.jar ru.vladshi.javalearning.Main ru/vladshi/javalearning/
```

И затем запустить jar файл:

```
java -jar Simulation2D.jar 
```
